	...
	START_TIME_TYPE start, long millisecsTimeout)
{
	...
	{	long s=0;
		Thread_unlock_mutex(mqttclient_mutex);
		s=millisecsTimeout - MQTTClient_elapsed(start);
		//printf("before: s : %ld\n",s);	//fix timeout reconnection   
		if(s<0)							//fix timeout reconnection
			s=30000;					//fix timeout reconnection every 30secs reconnect again
		printf("wait for %ld secs. \n",s/1000);	//fix timeout reconnection
...
exit:
	if (m->c->will)
	{
		free(m->c->will->topic);		//fix memory leak of will
		free(m->c->will->msg);			//fix memory leak of will
		free(m->c->will);
		m->c->will = NULL;
	}
	Thread_unlock_mutex(mqttclient_mutex);
